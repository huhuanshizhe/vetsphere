    ...
    try {
      ...
      if (paymentMethod === 'Stripe') {
          ...
          const response = await fetch('/api/payment/stripe/create-intent', {
            ...
            body: JSON.stringify({ orderId, amount: totalAmount, currency: 'CNY' }),
          });
          
          const result = await response.json();
          
          if (result.error) {
            // Check if it's a placeholder key error (test mode)
            if (result.error.includes('not configured') || result.error.includes('placeholder')) {
              setStatusMessage('Stripe test mode - simulating payment...');
              setTimeout(async () => {
                ...
                {(status === 'ready_for_payment' && paymentMethod === 'Stripe' && stripeClientSecret) ? (
                    ...
                    <div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                            {[
                            { id: 'Stripe', label: 'Credit Card / Link', sub: 'Secure Embedded Payment', icon: '💳', color: 'border-[#635BFF] bg-[#635BFF]/5 text-[#635BFF]' },
                            { id: 'Airwallex', label: 'Credit Card', sub: 'Airwallex Secure Form', icon: '🌏', color: 'border-vs bg-vs/5 text-vs' },
