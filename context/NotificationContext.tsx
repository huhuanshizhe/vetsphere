...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Trigger: When Supplier changes order status to "Shipped"
    ...
    // 2. Listen for Community Interactions (Simulated)
    // We assume a 'notifications' table exists where backend logic inserts rows when someone replies.
    ...
    <NotificationContext.Provider value={{ notifications, unreadCount, addNotification, markAsRead, clearAll, toast, hideToast }}>
      ...
      {toast && (
          <div className="fixed top-24 right-6 z-[1000] animate-in slide-in-from-right fade-in duration-300">
              <div className="bg-white border-l-4 border-vs p-4 rounded-xl shadow-2xl w-80 flex items-start gap-3 relative">
