...
> = {
  specialty_hub: {
    ...
    collectionBrief: 'These pages anchor topical authority. Use them to connect specialties with procedures, training pathways, equipment readiness, and adjacent FAQ coverage.',
    detailLensLabel: 'Why this page exists',
    detailLensCopy: 'A specialty hub should orient the reader quickly, show the capability stack, and route them into deeper procedure or resource pages without losing clinical context.',
    usageChecklist: [
    ...
    ctaCopy: 'Pair this hub with procedure, compare, and resource pages in the same specialty so operators grow a coherent search cluster instead of one-off pages.',
  },
  procedure: {
    ...
    collectionBrief: 'Procedure pages target named clinical workflows with high commercial and educational intent. They should help readers understand what the workflow is, what it requires, and where to go next.',
    detailLensLabel: 'Reader mode',
    detailLensCopy: 'A procedure page should answer the workflow question fast, then expand into training, readiness, equipment implications, and adjacent decision points.',
    usageChecklist: [
    ...
    ctaCopy: 'Good procedure pages should hand readers into related training and decision-support content instead of trapping them in one long article.',
  },
  case: {
    ...
    collectionBrief: 'Case pages are best when they translate evidence and workflow choices into a concrete learning scenario. They should feel practical, sourced, and restrained.',
    detailLensLabel: 'Case reading mode',
    detailLensCopy: 'Use case pages to show how a clinic or surgeon should think through a situation, not to overclaim outcomes or present unsupported medical certainty.',
    usageChecklist: [
    ...
    ctaCopy: 'Case pages work best as credibility layers around procedure and specialty programs, especially for advanced audiences evaluating training depth.',
  },
  solution: {
    ...
    collectionBrief: 'Solution pages turn multiple components into one adoption story. Use them when the goal is to explain how training, equipment, and workflow design come together for a clinic.',
    detailLensLabel: 'Commercial framing',
    detailLensCopy: 'A strong solution page should stay high-trust while still helping the reader understand how capability building translates into real clinic planning decisions.',
    usageChecklist: [
    ...
    ctaCopy: 'Use adjacent resource and procedure pages to support the adoption story with practical evidence and implementation detail.',
  },
  faq_hub: {
    listTitle: 'FAQ Hubs',
    ...
    collectionBrief: 'FAQ hubs are fast-answer surfaces for GEO and search summaries. They should collapse repeated friction points into clear, skimmable blocks with links to deeper pages.',
    detailLensLabel: 'Answer behavior',
    detailLensCopy: 'Readers come to FAQ hubs for speed. Keep the structure tight, the claims grounded, and the exits to deeper pages obvious.',
    usageChecklist: [
    ...
    ctaCopy: 'FAQ hubs should reduce friction and clarify intent before routing readers into deeper educational or commercial pages.',
  },
  glossary_term: {
    listTitle: 'Glossary',
    ...
    collectionBrief: 'Glossary pages reinforce terminology clarity across the content graph. They should be short, precise, and heavily linked to deeper pages.',
    detailLensLabel: 'Definition behavior',
    detailLensCopy: 'A glossary term is not a long-form article. It should define the term, explain why it matters, and connect readers into the correct broader topic.',
    usageChecklist: [
    ...
    ctaCopy: 'A glossary term becomes more valuable when it sits inside a larger mesh of specialty, procedure, and resource content.',
  },
  compare_page: {
    ...
    collectionBrief: 'Compare pages serve investigation-stage users who are trying to choose between alternatives. They should make the decision logic visible without overstating certainty.',
    detailLensLabel: 'Decision logic',
    detailLensCopy: 'Use compare pages to show the criteria, tradeoffs, and workflow fit of each path. Keep the structure balanced, readable, and operationally useful.',
    usageChecklist: [
    ...
    ctaCopy: 'Strong compare guides should leave the reader with a clearer next step, whether that is training evaluation, equipment planning, or deeper procedural reading.',
  },
  resource: {
    listTitle: 'Resources',
    ...
    collectionBrief: 'Resource pages should feel practical and reusable. They work best for checklists, worksheets, and planning assets that help a clinic actually execute the next step.',
    detailLensLabel: 'Operator mode',
    detailLensCopy: 'Resources should turn strategy into action. Use simple structure, obvious sequencing, and enough context that a clinic team can use the page during real execution.',
    usageChecklist: [
    ...
    ctaCopy: 'The best resource pages are not just readable. They help a clinic team move from interest into preparation, coordination, and implementation.',
  },
};
...
function flattenTextContent(node: ReactNode): string {
  ...
  if (typeof node === 'object' && 'props' in node) {
    return flattenTextContent((node as { props?: { children?: ReactNode } }).props?.children ?? null);
  }
...
const MARKDOWN_COMPONENTS: Components = {
  h1: ({ children }) => {
    ...
    return (
      <h1 id={id} className="scroll-mt-28 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
        {children}
      </h1>
    );
  },
  h2: ({ children }) => {
    const id = slugifyAnchor(flattenTextContent(children));
    return (
      <h2 id={id} className="scroll-mt-28 pt-2 text-2xl font-semibold tracking-tight text-slate-950 sm:text-[2rem]">
        {children}
      </h2>
    );
  },
  ...
  ul: ({ children }) => <ul className="space-y-3 pl-6 text-base leading-8 text-slate-700 marker:text-emerald-600 list-disc">{children}</ul>,
  ol: ({ children }) => <ol className="space-y-3 pl-6 text-base leading-8 text-slate-700 marker:font-semibold list-decimal">{children}</ol>,
  ...
  th: ({ children }) => <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-white/88">{children}</th>,
  td: ({ children }) => <td className="px-4 py-3 align-top text-sm leading-7 text-slate-700">{children}</td>,
  pre: ({ children }) => <pre className="overflow-x-auto rounded-[28px] bg-slate-950 p-5 text-sm leading-7 text-slate-100">{children}</pre>,
  code: ({ children }) => <code className="rounded bg-slate-100 px-1.5 py-0.5 text-[0.95em] text-slate-800">{children}</code>,
};
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-16">
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-end">
          <div className="max-w-4xl">
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">{copy.listDescription}</p>
            <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">{copy.collectionBrief}</p>
          ...
          <div className="rounded-[30px] border border-slate-200 bg-white/90 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur">
            ...
            <div className="mt-5 grid grid-cols-3 gap-3">
              ...
              <div className="rounded-2xl bg-slate-50 px-4 py-3">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Fresh</p>
            ...
            <div className="mt-5 space-y-3 rounded-[26px] bg-slate-950 px-5 py-5 text-slate-100">
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-200">Collection intent</p>
              <p className="text-sm leading-7 text-slate-200">Each item in this collection should answer a distinct reader job while strengthening the wider VetSphere knowledge graph.</p>
            </div>
          </div>
        </div>
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-3 text-sm text-white/70">
                      ...
                      {featuredItem.primary_specialty && (
                        <span className="rounded-full bg-white/10 px-3 py-1 text-white/80">{featuredItem.primary_specialty}</span>
                      )}
                      {featuredItem.primary_procedure && (
                        <span className="rounded-full bg-white/10 px-3 py-1 text-white/80">{featuredItem.primary_procedure}</span>
                      )}
                    </div>
                    ...
                    <div className="mt-8 flex flex-wrap gap-3 text-sm text-white/72">
                      {featuredItem.target_audience && (
                        <span className="rounded-full border border-white/12 bg-white/6 px-3 py-1">Audience: {featuredItem.target_audience}</span>
                      )}
                      {featuredItem.search_intent && (
                        <span className="rounded-full border border-white/12 bg-white/6 px-3 py-1">Intent: {featuredItem.search_intent}</span>
                      )}
                      <span className="rounded-full border border-white/12 bg-white/6 px-3 py-1">
                        Updated {formatDisplayDate(featuredItem.published_at || featuredItem.updated_at)}
                      </span>
                    </div>
                  </div>

                  <div className="rounded-[28px] border border-white/12 bg-white/8 p-6">
                    <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-200">How to use it</p>
                ...
                {remainingItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-6 flex flex-wrap gap-2 text-sm text-slate-500">
                      ...
                      {item.search_intent && (
                        <span className="rounded-full bg-slate-50 px-3 py-1">{item.search_intent}</span>
                      )}
                    </div>
                  </Link>
                ))}
        ...
        <article className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1fr)_300px]">
            <div className="space-y-8">
              <section className="overflow-hidden rounded-[36px] border border-slate-200 bg-white shadow-[0_20px_70px_rgba(15,23,42,0.08)]">
                <div className="border-b border-slate-200 bg-[linear-gradient(135deg,rgba(16,185,129,0.10),rgba(255,255,255,0.92)_42%,rgba(15,23,42,0.04))] p-8 sm:p-10">
                  ...
                  {content.summary && (
                    <p className="mt-6 max-w-4xl text-lg leading-8 text-slate-700">{content.summary}</p>
                  )}

                  {content.opening_answer && (
                    <div className="mt-8 rounded-[28px] border border-emerald-100 bg-white/80 p-6 shadow-sm">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">Opening Answer</p>
                      <p className="mt-3 text-base leading-8 text-slate-800">{content.opening_answer}</p>
                    </div>
                  )}

                  <div className="mt-8 flex flex-wrap gap-3 text-sm text-slate-500">
                    {content.target_audience && (
                      <span className="rounded-full border border-slate-200 bg-white px-3 py-1.5">Audience: {content.target_audience}</span>
                    )}
                    {content.search_intent && (
                      <span className="rounded-full border border-slate-200 bg-white px-3 py-1.5">Intent: {content.search_intent}</span>
                    )}
                ...
                <div className="grid gap-6 bg-slate-50/80 p-8 sm:p-10 lg:grid-cols-[minmax(0,1fr)_280px]">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">How to use this page</p>
                    <p className="mt-3 max-w-3xl text-lg leading-8 text-slate-800">{copy.detailLensCopy}</p>
                    {sectionCards.length > 0 && (
                      <div className="mt-6 grid gap-4 sm:grid-cols-2">
                        {sectionCards.map((section) => (
                          ...
                          >
                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Section {section.depth}</p>
                  ...
                  <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">{copy.detailLensLabel}</p>
              ...
              <section className="rounded-[36px] border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
                <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-5">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Main body</p>
                    <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Structured page content</h2>
                  </div>
                  {headingAnchors.length > 0 && (
                    <p className="text-sm text-slate-500">{headingAnchors.length} navigable sections</p>
                  )}
                </div>
              ...
              {faqPairs.length > 0 && (
                <section className="rounded-[36px] border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
                  <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-5">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">FAQ layer</p>
                      <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Frequently Asked Questions</h2>
                    </div>
                    <p className="text-sm text-slate-500">{faqPairs.length} answer blocks</p>
                  </div>
                  <div className="mt-6 space-y-4">
                    {faqPairs.map((item) => (
                      <div key={item.question} className="rounded-[26px] border border-slate-200 bg-slate-50/80 p-5 sm:p-6">
              ...
              {referenceItems.length > 0 && (
                <section className="rounded-[36px] border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
                  <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-5">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Evidence trail</p>
                      <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Internal References</h2>
                    </div>
                    <p className="text-sm text-slate-500">{referenceItems.length} source items</p>
                  </div>
                  <div className="mt-6 grid gap-4 lg:grid-cols-2">
                    {referenceItems.map((reference, index) => (
                      <div key={`${reference.title}-${index}`} className="rounded-[26px] border border-slate-200 bg-slate-50/80 p-5">
                        <div className="flex items-center justify-between gap-4">
                          <h3 className="text-base font-semibold text-slate-950">{reference.title}</h3>
                          <span className="rounded-full bg-slate-950 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/90">
                            {reference.badge}
                          </span>
                        </div>
                        {reference.context && <p className="mt-3 text-sm leading-7 text-slate-700">{reference.context}</p>}
                        {reference.href && (
                          <a
                            href={reference.href}
              ...
              {exploreItems.length > 0 && (
                <section className="rounded-[36px] border border-slate-200 bg-white p-8 shadow-sm sm:p-10">
                  <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-5">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Explore next</p>
                      <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">More from {copy.listTitle}</h2>
                    </div>
                  </div>
                  <div className="mt-6 grid gap-4 lg:grid-cols-3">
                    {exploreItems.map((item) => (
                      ...
                      >
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{copy.singularLabel}</p>
                        <h3 className="mt-3 text-lg font-semibold text-slate-950 group-hover:text-emerald-700">{item.title}</h3>
                        <p className="mt-3 line-clamp-4 text-sm leading-7 text-slate-600">{item.summary || copy.collectionBrief}</p>
                      </Link>
                    ))}
                  </div>
                </section>
            ...
            <aside className="space-y-6 self-start xl:sticky xl:top-24">
              <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Page dossier</p>
                <dl className="mt-5 space-y-4 text-sm leading-7 text-slate-700">
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Locale</dt>
                    <dd className="mt-1 text-base text-slate-950">{content.locale.toUpperCase()}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Audience</dt>
                    <dd className="mt-1 text-base text-slate-950">{content.target_audience || 'General professional audience'}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Search intent</dt>
                    <dd className="mt-1 text-base text-slate-950">{content.search_intent || 'Knowledge building'}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Freshness</dt>
                    <dd className="mt-1 text-base text-slate-950">{formatDisplayDate(content.published_at || content.updated_at)}</dd>
                  </div>
                </dl>
              </div>
                ...
                <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <nav className="mt-5 space-y-3">
                    {headingAnchors.map((heading) => (
                      ...
                      >
                        <span className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">H{heading.depth}</span>
              ...
              <div className="rounded-[28px] border border-slate-900 bg-slate-950 p-6 text-white shadow-[0_24px_70px_rgba(15,23,42,0.22)]">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-200">{copy.ctaTitle}</p>
