...
export default function VerificationsPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/admin/verifications/${confirmDialog.verification.id}`, {
        method: 'POST',
        headers: { 
  ...
  return (
    <>
      ...
      {/* Page Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">医生认证审核</h1>
                    ...
                    <tr key={v.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{v.displayName || v.user?.displayName || '未设置昵称'}</p>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-sm text-slate-700">{v.contact || v.mobile || v.user?.mobile || '-'}</p>
                        {v.siteCode && (
                          <span className="text-[9px] font-bold text-slate-400">{v.siteCode.toUpperCase()}</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
