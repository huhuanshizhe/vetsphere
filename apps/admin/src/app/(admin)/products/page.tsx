...
function getDisplayName(p: Product): string {
  switch (p.publish_language) {
    ...
    case 'th':
      return p.name_th || p.name || '(Unnamed)';
    case 'ja':
      return p.name_ja || p.name || '(Unnamed)';
    default:
      return p.name || p.name_en || '(未命名)';
  }
}
...
function isPublishedToSite(product: Product, siteCode: string): boolean {
  return (
    product.site_views?.some(
      (sv) =>
        sv.site_code === siteCode && sv.publish_status === 'published' && sv.is_enabled === true,
    ) ?? false
  );
}
      ...
      {open && (
        ...
        >
          ...
          {status === 'draft' && selfOwned && (
            <>
              <Divider />
              {!isOnCN && adminSite !== 'intl' && (
                <Item
                  onClick={() => onPublish('cn')}
                  icon={iconGlobe}
                  label="发布中国站"
                  color="text-blue-600 hover:bg-blue-50"
                />
              )}
              {!isOnINTL && adminSite !== 'cn' && (
                <Item
                  onClick={() => onPublish('intl')}
                  icon={iconGlobe}
                  label="发布国际站"
...
export default function AdminProductsPage() {
  ...
  const categoryDependency = categoryFilter ? categories.length : 0;
      ...
      try {
        ...
        const countResults = await Promise.all(
          statuses.map((s) => {
            ...
            if (siteProductIds !== null)
              q = q.in('id', siteProductIds.length > 0 ? siteProductIds : ['__none__']);
            return q;
          }),
        );
        ...
        if (categoryFilter) {
          ...
          if (categories.length === 0) {
            if (cancelled) return;
            setLoading(false);
            return; // Will re-run when categories load if a category filter is active.
          }
        ...
        if (debouncedSearch.trim()) {
          ...
          query = query.or(
            `name.ilike.%${kw}%,name_en.ilike.%${kw}%,sku_code.ilike.%${kw}%,brand.ilike.%${kw}%`,
          );
        }
        // Sort
        switch (sortBy) {
  ...
  const updateStatusCounts = useCallback((fromStatus: string, toStatus: string) => {
    setStatusCounts((prev) => {
      ...
      if (fromStatus && fromStatus !== 'all' && fromStatus in next) {
        next[fromStatus as keyof StatusCounts] = Math.max(
          0,
          (next[fromStatus as keyof StatusCounts] || 0) - 1,
        );
      }
          ...
          return (
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p
                  className="font-medium text-slate-900 text-sm leading-5 line-clamp-1"
                  title={name}
                >
                  {name}
                </p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  {product.sku_code && (
                    <span className="text-xs text-slate-400 font-mono">{product.sku_code}</span>
                  )}
                </div>
                {langs.length > 0 && (
                  <div className="flex gap-1 mt-1">
                    {langs.map((l) => (
                      <span
                        key={l}
                        className="text-[10px] leading-none px-1 py-0.5 rounded bg-slate-100 text-slate-500 font-medium"
                      >
                        {l}
                      </span>
                    ))}
      ...
      },
      {
        ...
        render: (_, product) => {
          ...
          return (
            <div>
              <span
                className={`text-sm font-medium ${stock <= 0 ? 'text-red-500' : stock < 10 ? 'text-amber-600' : 'text-slate-900'}`}
              >
                {stock}
              </span>
      ...
      },
      {
        ...
        render: (_, product) => {
          ...
          return (
            <div>
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${cfg?.className || 'bg-gray-100 text-gray-600'}`}
              >
        ...
        render: (_, product) => {
          ...
          return (
            <div className="flex gap-1">
              <span
                className={`px-1.5 py-0.5 text-[11px] rounded font-medium ${cn ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-400'}`}
              >
                CN
              </span>
              <span
                className={`px-1.5 py-0.5 text-[11px] rounded font-medium ${intl ? 'bg-purple-50 text-purple-600' : 'bg-slate-50 text-slate-400'}`}
              >
                INTL
              </span>
            </div>
      ...
      },
      {
        ...
        render: (_, product) => (
          <span
            ...
            title={new Date(product.created_at).toLocaleString('zh-CN')}
          >
      ...
      <div className="max-w-[1600px] mx-auto p-6">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">管理所有产品的审核、发布与下架</p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={() => router.push('/products/document-import')}
        ...
        <div className="flex gap-1.5 mb-5 overflow-x-auto pb-1">
          {STATUS_TABS.map((tab) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${active ? tab.dotColor : 'bg-slate-300'}`}
                />
                {tab.label}
                <span
                  className={`text-xs tabular-nums ${active ? 'text-slate-600' : 'text-slate-400'}`}
                >
                  {count}
                </span>
              </button>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {['draft', 'approved', 'offline', 'all'].includes(statusFilter) && (
                <>
                  {currentSite !== 'intl' && (
                    <button
                      onClick={() => setBulkConfirm({ action: 'publish', siteCodes: ['cn'] })}
                  ...
                  {currentSite !== 'cn' && (
                    <button
                      onClick={() => setBulkConfirm({ action: 'publish', siteCodes: ['intl'] })}
              ...
              {statusFilter === 'draft' && (
                <button
                  onClick={() => setBulkConfirm({ action: 'submit_review' })}
              ...
              {statusFilter === 'pending_review' && (
                <>
                  ...
                  <button
                    onClick={() => setBulkConfirm({ action: 'reject' })}
              ...
              {(statusFilter === 'published' || statusFilter === 'all') && (
                <>
                  {currentSite !== 'intl' && (
                    <button
                      onClick={() => setBulkConfirm({ action: 'offline', siteCodes: ['cn'] })}
                      className="px-3 py-1.5 bg-slate-500 text-white text-xs font-medium rounded-md hover:bg-slate-600 transition-colors"
                    >
                      下架中国站
                    </button>
                  )}
                  {currentSite !== 'cn' && (
                    <button
                      onClick={() => setBulkConfirm({ action: 'offline', siteCodes: ['intl'] })}
                      className="px-3 py-1.5 bg-slate-500 text-white text-xs font-medium rounded-md hover:bg-slate-600 transition-colors"
                    >
                      下架国际站
                    </button>
              ...
              <button
                ...
                className="px-3 py-1.5 text-slate-600 text-xs font-medium rounded-md hover:bg-slate-100 transition-colors"
              >
                取消
              </button>
            </div>
            ...
            <div className="flex items-center justify-between px-6 py-3 border-t border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-3">
                <p className="text-sm text-slate-500">
                  共 <span className="font-medium text-slate-700">{totalCount}</span> 条， 显示{' '}
              ...
              <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
                ...
                {bulkConfirm.action === 'reject' && (
                  <textarea
                    ...
                    onChange={(e) =>
                      setBulkConfirm((prev) => (prev ? { ...prev, reason: e.target.value } : null))
                    }
                    placeholder="请输入拒绝原因..."
                    rows={3}
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 text-sm text-white rounded-lg transition-colors font-medium ${confirmColor}`}
                  >
                    确认
                  </button>
                </div>
