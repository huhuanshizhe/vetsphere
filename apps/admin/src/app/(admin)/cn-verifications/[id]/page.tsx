...
export default function CnVerificationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusLabels: Record<string, { label: string; color: string }> = {
    ...
    submitted: { label: '待审核', color: 'text-yellow-400' },
    under_review: { label: '审核中', color: 'text-blue-400' },
  ...
  if (error || !verification) {
    return (
      <Card className="text-center py-12">
        <p className="text-red-400 mb-4">{error || '未找到认证申请'}</p>
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">认证详情</h1>
            <p className="text-slate-400 text-sm">申请ID: {verification.id}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusInfo.color} bg-white/5`}>
            {statusInfo.label}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-white font-medium">{verification.realName || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-slate-400">手机号</p>
                <p className="text-white font-medium">{verification.user?.mobile || '-'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white font-medium">{verification.identity?.identityGroup || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-slate-400">所在单位</p>
                <p className="text-white font-medium">{verification.organizationName || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-slate-400">职位/角色</p>
                <p className="text-white font-medium">{verification.positionTitle || '-'}</p>
              </div>
            </div>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {verification.specialtyTags.map((tag, i) => (
                    <span key={i} className="px-2 py-1 text-xs bg-blue-500/20 text-blue-400 rounded">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          {/* Documents */}
          <Card>
            <h2 className="text-lg font-semibold text-white mb-4">证明材料</h2>
            {verification.documents && verification.documents.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {verification.documents.map((doc) => (
                  <div key={doc.id} className="bg-white/5 rounded-lg p-3">
                    ...
                    {doc.documentUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center h-32 bg-white/5 rounded text-slate-400 hover:text-white transition-colors"
                      >
          ...
          {/* Audit Logs */}
          <Card>
            <h2 className="text-lg font-semibold text-white mb-4">审核记录</h2>
                ...
                {verification.auditLogs.map((log) => (
                  <div key={log.id} className="flex gap-4 pb-4 border-b border-white/5 last:border-0 last:pb-0">
                    ...
                    <div className="flex-1">
                      ...
                      {log.reason && (
                        <p className="text-sm text-red-400 mt-1">{log.reason}</p>
                      )}
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-white font-medium">{verification.profile?.displayName || '未设置昵称'}</p>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-white">{verification.user?.status || '-'}</span>
              </div>
              <div className="flex justify-between">
          ...
          {verification.status === 'rejected' && verification.rejectReason && (
            <Card className="border-red-500/30">
              <h2 className="text-lg font-semibold text-red-400 mb-2">拒绝原因</h2>
              <p className="text-slate-300">{verification.rejectReason}</p>
            </Card>
          )}
          ...
          {canReview && (
            <Card>
              <h2 className="text-lg font-semibold text-white mb-4">审核操作</h2>
