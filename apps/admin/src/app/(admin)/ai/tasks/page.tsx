  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">AI 任务模板</h1>
          <p className="text-slate-500 mt-1">管理 AI 任务的 Prompt 模板和模型配置</p>
        </div>
            ...
            <TableContainer>
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-600">任务</th>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-600">参数</th>
                ...
                <tbody className="divide-y divide-slate-700/50">
                  {tasks.map((task) => (
                    <tr key={task.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-500 text-sm">{task.category || '-'}</td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                          task.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-500/20 text-slate-500'
                        }`}>
      ...
      {showEditDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-950 border border-slate-200/50 rounded-xl p-6 max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">{editingTask ? '编辑任务模板' : '新增任务模板'}</h3>
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-slate-500">Prompt 模板 *</label>
                <textarea
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 font-mono min-h-[200px]"
