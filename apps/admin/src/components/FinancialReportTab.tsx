...
const FinancialReportTab: React.FC<FinancialReportTabProps> = ({ orders }) => {
  ...
  const columns: Column<Order>[] = [
    { key: 'id', header: '订单号', render: (v) => <span className="font-mono font-bold text-slate-900">#{String(v).slice(0, 8)}</span> },
    { key: 'customerEmail', header: '客户', render: (v, row) => <span className="text-slate-500">{v || row.customerName || '-'}</span>, hideOnMobile: true },
    {
      key: 'items',
      header: '类型',
      render: (_, row) => (
    ...
    { key: 'totalAmount', header: '金额', render: (v) => <span className="font-bold text-slate-900">¥{(v || 0).toLocaleString()}</span> },
    { key: 'date', header: '日期', render: (v) => <span className="text-slate-500">{v}</span>, hideOnMobile: true },
    {
      ...
      render: (v) => {
        ...
        return <span className={`text-xs font-bold px-2 py-1 rounded ${colors[v] || 'bg-white text-slate-500'}`}>{v}</span>;
      },
    },
  ];
  ...
  const mobileCard = (order: Order) => (
    <div className="bg-black/30 border border-white/5 p-4 rounded-xl">
      <div className="flex justify-between items-start mb-2">
        <div>
          <p className="font-mono font-bold text-slate-900">#{order.id.slice(0, 8)}</p>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <h3 className="font-bold text-lg text-slate-900">财务报表</h3>
      ...
      <div className="bg-black/20 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-4 sm:p-5 border-b border-white/5">
          <h4 className="font-bold text-slate-900">订单明细</h4>
