...
const ROLE_COLORS: Record<string, string> = {
  ...
  Doctor: 'bg-slate-800 text-slate-400',
};
  ...
  const columns: Column<AdminUser>[] = [
    {
      ...
      render: (value, row) => (
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-white truncate">{value || 'User'}</span>
        </div>
      ),
    },
    { key: 'email', header: '邮箱', render: (v) => <span className="text-slate-400 text-sm">{v}</span>, hideOnMobile: true },
    {
      key: 'role',
      header: '角色',
      render: (v) => (
        <span className={`text-xs font-bold px-2 py-1 rounded ${ROLE_COLORS[v] || 'bg-slate-800 text-slate-400'}`}>
          {v}
        </span>
      ),
    },
  ...
  const mobileCard = (user: AdminUser) => (
    <div className="bg-black/30 border border-white/5 p-4 rounded-xl">
      ...
      <div className="flex justify-between items-center">
        <span className={`text-xs font-bold px-2 py-1 rounded ${ROLE_COLORS[user.role] || 'bg-slate-800 text-slate-400'}`}>
          {user.role}
        </span>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <h3 className="font-bold text-lg text-white">用户管理</h3>
        <select
          value={roleFilter}
          onChange={(e) => setRoleFilter(e.target.value)}
          className="bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-300 focus:border-emerald-500 outline-none min-h-[44px]"
        >
