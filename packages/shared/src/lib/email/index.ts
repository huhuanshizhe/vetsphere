 ...
 * - EMAIL_FROM: Default sender email (configured per app via NEXT_PUBLIC_APP_URL)
 */
...
<body style="margin: 0; padding: 0; background-color: #f8fafc; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  ...
  refundStatus: (details: {
    ...
    subject: `Refund ${details.status === 'completed' ? 'Processed' : details.status === 'approved' ? 'Approved' : 'Update'} - Order #${details.orderId}`,
    html: `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Refund Update</title></head>
<body style="margin: 0; padding: 0; background-color: #f8fafc; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    <tr>
      <td>
        <div style="background: white; border-radius: 16px; padding: 40px; text-align: center;">
          <div style="width: 80px; height: 80px; background: ${details.status === 'rejected' ? '#fee2e2' : '#d1fae5'}; border-radius: 50%; margin: 0 auto 20px; display: flex; align-items: center; justify-content: center;">
            <span style="font-size: 40px;">${details.status === 'rejected' ? '✕' : '✓'}</span>
          </div>
          <h2 style="color: ${details.status === 'rejected' ? '#dc2626' : '#10B981'}; margin-bottom: 10px;">
            Refund ${details.status === 'completed' ? 'Completed' : details.status === 'approved' ? 'Approved' : 'Declined'}
          </h2>
          <p style="color: #64748b;">Order #${details.orderId}</p>
          <p style="color: #64748b;">Refund #${details.refundId}</p>
          ${details.status !== 'rejected' ? `
          <p style="font-size: 28px; font-weight: bold; color: #1e293b; margin: 20px 0;">¥${details.amount.toLocaleString()}</p>
          <p style="color: #475569;">${details.status === 'completed' ? 'has been refunded to your original payment method.' : 'Your refund has been approved and will be processed shortly.'}</p>
          ` : `
          <p style="color: #475569; margin-top: 20px;">Unfortunately, your refund request could not be approved.</p>
          ${details.reason ? `<p style="color: #64748b; font-style: italic;">"${details.reason}"</p>` : ''}
          <p style="color: #475569; margin-top: 20px;">If you have questions, please contact our support team.</p>
          `}
          <p style="color: #475569; margin-top: 20px;">Dear ${details.customerName}, thank you for your patience.</p>
        </div>
      </td>
    </tr>
  </table>
    ...
    text: `Refund ${details.status} for Order #${details.orderId}. Amount: ¥${details.amount}${details.reason ? `. Reason: ${details.reason}` : ''}`
  })
};
...
export async function sendEmail(options: EmailOptions): Promise<EmailResult> {
  ...
  const from = options.from || process.env.EMAIL_FROM || `VetSphere <noreply@${process.env.NEXT_PUBLIC_DOMAIN || 'vetsphere.net'}>`;
...
export async function sendWelcomeEmail(email: string, userName: string): Promise<EmailResult> {
  const template = emailTemplates.welcome(userName, `${process.env.NEXT_PUBLIC_APP_URL || 'https://vetsphere.net'}/dashboard`);
...
export async function sendRefundStatusEmail(email: string, details: Parameters<typeof emailTemplates.refundStatus>[0]): Promise<EmailResult> {
  const template = emailTemplates.refundStatus(details);
  return sendEmail({ to: email, ...template });
}
