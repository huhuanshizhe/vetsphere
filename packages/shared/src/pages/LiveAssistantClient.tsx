...
const LiveAssistant: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-950 text-white flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className={`absolute w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 bg-emerald-500/20 rounded-full blur-[80px] sm:blur-[100px] transition-all duration-300 transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 ${status === 'listening' ? 'scale-125 opacity-30' : 'scale-100 opacity-10'}`}></div>

        {/* Top Bar */}
        <div className="absolute top-0 left-0 right-0 p-4 sm:p-6 md:p-8 flex justify-between items-center z-10">
            <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-red-500 animate-pulse"></div>
                <span className="font-mono text-[10px] sm:text-xs font-bold uppercase tracking-widest text-red-500">Live • Surgical Mode</span>
            </div>
            <button onClick={() => router.push('/ai')} className="px-4 sm:px-6 py-2 sm:py-3 border border-white/20 rounded-full text-xs font-bold uppercase hover:bg-white/10 transition-all min-h-[40px]">
                Exit
            </button>
        </div>

        {/* Main Visualizer */}
        <div className="relative z-10 flex flex-col items-center space-y-8 sm:space-y-10 md:space-y-12 px-4">
            
            <div className="relative">
                {/* Central Orb */}
                <button 
                    onClick={isConnected ? handleDisconnect : startSession}
                    className={`w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 rounded-full flex items-center justify-center transition-all duration-500 relative
                        ${isConnected 
                            ? (status === 'speaking' ? 'bg-white shadow-[0_0_40px_rgba(255,255,255,0.5)] sm:shadow-[0_0_50px_rgba(255,255,255,0.5)]' : 'bg-vs shadow-[0_0_25px_rgba(0,168,132,0.6)] sm:shadow-[0_0_30px_rgba(0,168,132,0.6)]') 
                            : 'bg-slate-800 border-2 border-slate-700 hover:border-vs'
                        }`}
                >
                    {isConnected ? (
                        <span className="text-3xl sm:text-4xl">{status === 'speaking' ? '🔊' : '👂'}</span>
                    ) : (
                        <span className="text-3xl sm:text-4xl text-slate-500">🎙️</span>
                    )}
                    ...
                    {status === 'listening' && (
                        <>
                            ...
                            <div className="absolute -inset-3 sm:-inset-4 border border-vs rounded-full animate-ping opacity-10 animation-delay-500"></div>
                        </>
                    )}
                </button>
            </div>
            ...
            <div className="text-center space-y-3 sm:space-y-4 max-w-sm sm:max-w-md">
                ...
                <p className="text-white/50 font-mono text-xs sm:text-sm mx-auto">
                    {status === 'disconnected' 
            ...
            {isConnected && (
                <div className="flex items-center gap-0.5 sm:gap-1 h-10 sm:h-12">
                    {[...Array(10)].map((_, i) => (
                        <div 
                            key={i} 
                            className={`w-1 sm:w-1.5 bg-white/50 rounded-full transition-all duration-100 ease-in-out`}
                            style={{ 
