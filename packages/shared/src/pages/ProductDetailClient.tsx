...
const ProductDetailClient: React.FC<ProductDetailClientProps> = ({ productId }) => {
  ...
  const getStockStatusText = (status: string, qty?: number) => {
    if (language === 'zh') {
      switch (status) {
        case 'In Stock': return '现货';
        case 'Low Stock': return qty ? `仅剩 ${qty} 件` : '库存紧张';
        case 'Out of Stock': return '缺货';
        default: return status;
      }
    }
    switch (status) {
      case 'Low Stock': return qty ? `Only ${qty} left` : 'Low Stock';
      default: return status;
    }
  };
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-100">
              ...
              <div className="pt-4 border-t border-slate-100 space-y-2">
                <div className="flex items-center gap-2 text-sm text-slate-500">
                  <span>&#128666;</span>
                  <span className="font-bold">{language === 'zh' ? '国内包邮 (顺丰速运)' : 'Free domestic shipping (SF Express)'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-slate-500">
                  <span>&#127758;</span>
                  <span className="font-bold">{language === 'zh' ? '海外发货 运费另计' : 'International shipping calculated at checkout'}</span>
                </div>
              </div>
            </div>
